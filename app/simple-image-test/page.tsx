...
export default function SimpleImageTest() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <h1 className="text-3xl font-bold mb-8">{t('imageTest.title')}</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {imagePaths.map((path, index) => (
          <div key={index} className="border border-gray-700 rounded-lg p-4">
            <p className="text-sm text-gray-400 mb-2 break-all">{t('imageTest.path')}: {path}</p>
            <div className="h-32 bg-gray-800 rounded flex items-center justify-center overflow-hidden">
              <img
                src={path}
                alt={`${t('imageTest.test')} ${index + 1}`}
            ...
            <div className="mt-2 text-sm">
                {t('imageTest.status')}: {imageLoaded[path] === undefined ? t('imageTest.loading') : 
                     imageLoaded[path] ? t('imageTest.loadedSuccess') : t('imageTest.loadedFailed')}
              </div>
          </div>
        ))}
      </div>
